 ...
 * @param {Array} rules - Rules dari database
 * @returns {Array} Sorted results dengan CF score per penyakit
 */
function runCFInference(answers, rules) {
 ...
 */
function getPrimaryDiagnosis(answers, rules) {
  const results = runCFInference(answers, rules);
 ...
 */
function interpretCFLevel(percentage) {
  if (percentage >= 70) {
    return {
      label: "Tinggi",
      color: "text-red-600",
